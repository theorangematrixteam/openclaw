"""
Nano Banana Inpainter - Single workflow using SAM3 + Crop + Z-Image + Stitch.
Uses Jinay's exact workflow structure with Z-Image inpainting in the middle.

Usage:
  python nanobanana_inpaint.py --input image.png --prompt "red t-shirt and blue shorts"
  python nanobanana_inpaint.py --input image.png --prompt "green shirt" --sam-prompt "shirt"
"""
...
def inpaint(image_path, prompt, sam_prompt="shirt shorts clothing", seed=42):
    """Full pipeline: SAM3 mask → Crop → Z-Image inpaint → Stitch."""
    
    # Upload image
    print("Uploading image...")
    img_filename = upload_image(image_path)
    
    # Build combined workflow: SAM3 + Crop + Z-Image + Stitch
    workflow = {
        # Load original image
        "4": {
        ...
        "8": {
            ...
            "_meta": {"title": "SAM3 Segment Clothing"}
        },
        # Crop around mask
        "3": {
            "inputs": {
        ...
        # Z-Image model
        "46": {
            "inputs": {"unet_name": "z-image\\z_image_turbo_bf16.safetensors", "weight_dtype": "default"},
        ...
        "50": {
            "inputs": {"pixels": ["3", 1], "vae": ["40", 0]},
            "class_type": "VAEEncode",
            "_meta": {"title": "VAE Encode"}
        },
        # Set noise mask from cropped mask
        "51": {
            "inputs": {"samples": ["50", 0], "mask": ["3", 2]},
        ...
        # Stitch back
        "5": {
            "inputs": {"stitcher": ["3", 0], "inpainted_image": ["43", 0]},
            "class_type": "InpaintStitchImproved",
            "_meta": {"title": "Inpaint Stitch"}
        },
        # Save result
        "148": {
            "inputs": {"filename_prefix": "inpaint", "images": ["5", 0]},
            "class_type": "SaveImage",
            "_meta": {"title": "Save Result"}
        },
        # Save mask preview
        "149": {
            "inputs": {"filename_prefix": "sam3_mask", "images": ["8", 0]},
            "class_type": "SaveImage",
    ...
    # Queue
    print(f"Running full pipeline (SAM3: '{sam_prompt}', seed: {seed})...")
        ...
        if "images" in node_output:
            for img_info in node_output["images"]:
                ...
                try:
                    ...
                    if "inpaint" in fn:
                        out = os.path.join(OUTPUT_DIR, f"inpaint_{base_name}.png")
                    elif "mask" in fn:
                        out = os.path.join(OUTPUT_DIR, f"mask_{base_name}.png")
                    else:
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Nano Banana Inpainter - SAM3 + Crop + Z-Image + Stitch")
    ...
    print(f"SAM3: {args.sam_prompt}")
    
    result = inpaint(args.input, args.prompt, sam_prompt=args.sam_prompt, seed=args.seed)
