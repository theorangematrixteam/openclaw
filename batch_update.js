...
#Proofit #PreMonsoon #Waterproofing #PropertyCare`},
  {row: 14, text: `Rs 5,000 inspection.
Or Rs 2 lakh in repairs?
...
// Build all values into one JSON file
const allValues = captions.map(c => [c.text]);
const range = "proofit!K3:K25";

fs.writeFileSync('batch_values.json', JSON.stringify(allValues));
console.log('Wrote batch_values.json');

// Now run gog with the file
const cmd = `gog sheets update ${sheetId} "${range}" --values-json @batch_values.json --input USER_ENTERED --no-input`;
