...
for (const cap of captions) {
  ...
  const escapedText = cap.text.replace(/"/g, '\\"').replace(/\n/g, '\\n');
  const json = `[["${escapedText}"]]`;
  const cmd = `gog sheets update ${sheetId} "${range}" --values-json '${json}' --input USER_ENTERED --no-input`;
  console.log(`Updating row ${cap.row}...`);
  try {
    execSync(cmd, { shell: 'powershell.exe', stdio: 'inherit' });
  } catch (e) {
    console.error(`Failed on row ${cap.row}:`, e.message);
  }
}
