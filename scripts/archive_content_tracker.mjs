...
const header = ["Client","Month","Post Date","Platform","Content Type","Bucket","Topic / Hook","Status","Creation Owner","Draft Link","Approval Status","Scheduled Status","Posted Status","Notes"];
...
const archiveRows = allRows.filter(r => r[7] === 'Approved');
const activeRows = allRows.filter(r => r[7] !== 'Approved');

function run(args) {
  const res = spawnSync(GOG, args, { stdio: 'pipe', encoding: 'utf8' });
  if (res.stdout) process.stdout.write(res.stdout);
  if (res.stderr) process.stderr.write(res.stderr);
  if ((res.status ?? 1) !== 0) process.exit(res.status ?? 1);
}

// add archive tab if missing; ignore failure if already exists
spawnSync(GOG, ['sheets', 'add-tab', SHEET, 'content archive'], { stdio: 'ignore' });

run(['sheets', 'clear', SHEET, "'content tracker'!A1:N200"]);
run(['sheets', 'clear', SHEET, "'content archive'!A1:N200"]);
run(['sheets', 'update', SHEET, "'content tracker'!A1:N1", '--values-json', JSON.stringify([header])]);
run(['sheets', 'update', SHEET, "'content archive'!A1:N1", '--values-json', JSON.stringify([header])]);
if (activeRows.length) run(['sheets', 'append', SHEET, "'content tracker'!A2:N", '--values-json', JSON.stringify(activeRows)]);
if (archiveRows.length) run(['sheets', 'append', SHEET, "'content archive'!A2:N", '--values-json', JSON.stringify(archiveRows)]);
run(['sheets', 'freeze', SHEET, '--sheet', 'content tracker', '--rows', '1']);
run(['sheets', 'freeze', SHEET, '--sheet', 'content archive', '--rows', '1']);
